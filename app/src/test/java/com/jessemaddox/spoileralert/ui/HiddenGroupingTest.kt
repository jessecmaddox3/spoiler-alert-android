...
class HiddenGroupingTest {
    ...
    @Test fun `a larger disjoint snapshot cannot erase earlier messages`() {
        val earlier = item(1, "demo.chat", "Demo chat", "Invented group", base,
            text = "First\nSecond", messageCount = 2)
        val later = item(1, "demo.chat", "Demo chat", "Invented group", base + min,
            text = "Third\nFourth\nFifth", messageCount = 3)
        val displayed = HiddenGrouping.group(listOf(earlier, later)).single().conversations.single().displayItems
        assertEquals(listOf(earlier.id, later.id), displayed.map { it.id })
    }

    @Test fun `identical text in separate protection sessions remains separate`() {
        val earlier = item(1, "demo.chat", "Demo chat", "Invented group", base,
            text = "First\nSecond", messageCount = 2).copy(captureSessionId = "first", ownerSessionId = "first")
        val later = earlier.copy(id = earlier.id + 1, postedAtMillis = base + min,
            captureSessionId = "second", ownerSessionId = "second")
        val displayed = HiddenGrouping.group(listOf(earlier, later)).single().conversations.flatMap { it.displayItems }
        assertEquals(setOf(earlier.id, later.id), displayed.map { it.id }.toSet())
    }
    ...
    fun `multi-item burst from same conversation collapses into one group sorted ascending`() {
        val groups = HiddenGrouping.group(
            listOf(
                item(1, "com.wa", "WhatsApp", "Demo sports chat", base + 3 * min),
                item(1, "com.wa", "WhatsApp", "Demo sports chat", base + 1 * min),
                item(1, "com.wa", "WhatsApp", "Demo sports chat", base + 2 * min),
            )
        )
        assertEquals(1, groups[0].conversations.size)
        val c = groups[0].conversations[0]
    ...
    fun `distinct conversations in same app stay separate`() {
        val groups = HiddenGrouping.group(
            listOf(
                item(1, "com.wa", "WhatsApp", "Demo sports chat", base),
    ...
    fun `rolling conversation titles group and cumulative counts are not added twice`() {
        val convo = HiddenGrouping.group(
            listOf(
                item(1, "com.wa", "WhatsApp", "Paper Kite Workshop (2 messages)", base,
                    messageCount = 2),
                item(1, "com.wa", "WhatsApp", "Paper Kite Workshop (3 messages): Theo", base + min,
                    messageCount = 3),
            )
        ).single().conversations.single()

        assertEquals("Paper Kite Workshop", convo.conversationLabel)
    ...
    fun `rolling count in a legacy notification title repairs the displayed message count`() {
        val convo = HiddenGrouping.group(
            listOf(
                item(
                    1, "com.wa", "WhatsApp", "Paper Kite Workshop", base,
                    title = "Paper Kite Workshop (5 messages): Theo Finch", messageCount = 2,
                ),
            )
        ).single().conversations.single()
