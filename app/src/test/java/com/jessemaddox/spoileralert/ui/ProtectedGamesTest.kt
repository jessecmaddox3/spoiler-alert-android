...
class ProtectedGamesTest {
    ...
    private val game = GameEntity(
        id = "fiction-summary", shieldId = 7, name = "Amber Owls at Violet Finches",
        shortName = "AMB @ VIO", startMillis = 100L, completed = false, fetchedAtMillis = 90L,
    )
    ...
    @Test fun `team cached game reaches Home even when league discovery row is missing`() {
        ...
        assertEquals(listOf(ProtectedLiveGame(
            eventId = "fiction-summary", leagueId = "mls", name = game.name,
            shortName = "AMB @ VIO", startMillis = 100L, completed = false, label = null,
        )), out)
    }
    ...
    @Test fun `multi-day game shield reaches Home without recent team cache row`() {
        val gameShield = ShieldEntity(
            id = 8, name = "Meadow Open", aliasesJson = "[]", kind = "GAME",
            gameEventId = "open", armed = true,
        )
        val discovery = LeagueGameEntity(
            eventId = "open", leagueId = "golf", name = "Meadow Open",
            shortName = "Meadow Open", startMillis = 10L, completed = false,
