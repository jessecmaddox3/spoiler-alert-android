...
class MessageAttributionTest {
    @Test fun `message line prefixes known sender`() {
        assertEquals("Ellis Cedar: hi", messageLine("Ellis Cedar", "hi"))
    }
    ...
    @Test fun `rolling chat titles canonicalize to one conversation`() {
        assertEquals("Paper Kite Workshop", canonicalConversationTitle("Paper Kite Workshop (3 messages): Ellis"))
        assertEquals("Mira", canonicalConversationTitle("Mira (2 new messages)"))
    }
}
