...
enum class ScoreQuestion {
    STARTED, PROGRESS, ANY_SCORE, TIED, CLOSE, BLOWOUT, OVER,
...
object SpoilerFreeScore {
    ...
    fun answer(snapshot: ScoreSnapshot, question: ScoreQuestion): ScoreAnswer {
        ...
        return when (question) {
            ScoreQuestion.STARTED, ScoreQuestion.PROGRESS,
    ...
    private fun soccerProgress(snapshot: ScoreSnapshot): String? {
        val minute = snapshot.displayClock
            ?.substringBefore(':')
            ?.filter(Char::isDigit)
            ?.toIntOrNull()
            ?: return null
        ...
        return "$segment, ${ordinal(minute)} minute."
    }
