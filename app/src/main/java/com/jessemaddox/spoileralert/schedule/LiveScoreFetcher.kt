...
class LiveScoreFetcher(private val fetcher: ScheduleFetcher = ScheduleFetcher()) {
    ...
    suspend fun catchUp(
        leagueId: String, eventId: String, startMillis: Long, request: CatchUpRequest,
    ): String? {
        val scoreboard = LEAGUE_SCOREBOARDS.firstOrNull { it.leagueId == leagueId } ?: return null
        if (leagueId in setOf("epl", "mls", "soccer")) {
            val state = fetcher.fetchEventState(scoreboard.path, leagueId, eventId) ?: return null
            return if (request.question == CatchUpQuestion.SUMMARY)
                CatchUpSummary.answer(state.snapshot, state.timeline, request)
            else SoccerGoalGuide.answer(state.timeline, request)
        }
        if (request.question != CatchUpQuestion.SUMMARY) return null
        val snapshot = fetch(leagueId, eventId, startMillis, forceRefresh = true) ?: return null
        return CatchUpSummary.answer(snapshot, null, request)
    }
    ...
    private suspend fun timeline(
        ...
        synchronized(timelineCacheEntries) {
            if (result != null) timelineCacheEntries[cacheKey] = TimelineCacheEntry(result, now)
            else timelineCacheEntries.remove(cacheKey)
        }
        return result
    }
