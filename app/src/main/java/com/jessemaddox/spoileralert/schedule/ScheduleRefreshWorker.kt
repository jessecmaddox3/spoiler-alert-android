...
class ScheduleRefreshWorker(context: Context, params: WorkerParameters) :
     ...
     * saved teams. One bounded GET per configured feed over a rolling today → +8 days window. A failed/empty fetch keeps
     * that league's old cache; the prune then clears completed and already-started games
     * (with a grace so a game that just kicked off stays armable).
     */
    private suspend fun refreshLeagueGames(
        ...
        val dates = EspnScoreboard.datesParam(now, now + LEAGUE_WINDOW_MS)
        ...
        if (prune) db.leagueGameDao().pruneBefore(now - LEAGUE_STARTED_GRACE_MS)
    }
    ...
    companion object {
        ...
        /** Keep an in-progress game armable for a few hours after kickoff before pruning. */
        private const val LEAGUE_STARTED_GRACE_MS = 6L * 60 * 60 * 1000
