...
object MaskedNotifier {
    ...
    fun cancel(context: Context, vaultRowId: Long) {
        context.getSystemService(NotificationManager::class.java).cancel(idFor(vaultRowId))
    }
    ...
    fun reassign(context: Context, vaultRowId: Long, newShieldId: Long) {
        val nm = context.getSystemService(NotificationManager::class.java)
        val id = idFor(vaultRowId)
        val current = runCatching { nm.activeNotifications.firstOrNull { it.id == id } }
            .getOrNull() ?: return
        val updated = android.app.Notification.Builder.recoverBuilder(context, current.notification)
            .setGroup(ProtectionNotificationPolicy.groupKey(newShieldId))
            .setGroupAlertBehavior(android.app.Notification.GROUP_ALERT_SUMMARY)
            .build()
        nm.notify(id, updated)
    }
}
