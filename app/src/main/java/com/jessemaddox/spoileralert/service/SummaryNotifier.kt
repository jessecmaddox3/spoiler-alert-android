...
object SummaryNotifier {
    ...
    suspend fun refresh(context: Context) {
        ...
        val armed = db.shieldDao().armed()
        val nm = app.getSystemService(NotificationManager::class.java)
        nm.cancel(LEGACY_SUMMARY_ID)

        val activeIds = armed.mapTo(mutableSetOf()) {
            ProtectionNotificationPolicy.summaryId(it.id)
        }
        nm.activeNotifications
            .filter { ProtectionNotificationPolicy.isSummaryId(it.id) && it.id !in activeIds }
        ...
        armed.forEach { shield ->
            ...
            post(app, shield, hiddenCount, hasGame, now)
        }
    }
    ...
    private fun post(
        ...
        if (ProtectionNotificationPolicy.showExtend(deadline, nowMillis)) {
            val extendIntent = PendingIntent.getBroadcast(
                context,
                id,
                Intent(context, ExtendShieldReceiver::class.java)
                    .setAction(ExtendShieldReceiver.ACTION_EXTEND_SHIELD)
                    .putExtra(ExtendShieldReceiver.EXTRA_SHIELD_ID, shield.id),
                PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT,
            )
