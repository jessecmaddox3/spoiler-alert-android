...
fun SettingsScreen(
    ...
    val developmentBuild = remember(context) {
    ...
    val installedVersion = remember(context) {
        ...
        }.getOrDefault("Installed version unavailable")
    }
    ...
    Column(
        ...
        JessCard {
            Eyebrow("App updates", small = true)
            Text(installedVersion, style = MaterialTheme.typography.titleMedium, color = Blueberry)
            Text(
                if (developmentBuild) "This is a development install. Rebuild from source to update it. The official APK installs as a separate app and does not move this install's data."
                else "Open the latest release and download spoiler-alert.apk. Open the file and tap Update. Your saved teams and notifications stay on this phone when updating the same app.",
                style = MaterialTheme.typography.bodySmall,
                color = JessColors.subtle,
            )
            TextAction(if (developmentBuild) "View official releases" else "Download latest update", onClick = {
                openUpdateLink("https://github.com/jessecmaddox3/spoiler-alert-android/releases/latest")
            })
            TextAction("What's new", onClick = {
                openUpdateLink("https://github.com/jessecmaddox3/spoiler-alert-android/releases")
            })
            if (updateError) {
                Text("No browser is available. Visit the Spoiler Alert project on GitHub to download the update.",
                    style = MaterialTheme.typography.bodySmall, color = Blueberry)
            }
        }
        ...
        JessCard {
            ...
            Text(
                "Notification matching and hidden content stay on-device. Internet is used only for GET-only public schedules, event discovery, active-event completion checks, and spoiler-free game status.",
                style = MaterialTheme.typography.bodySmall,
                color = JessColors.subtle,
            )
            var showHowItWorks by rememberSaveable { mutableStateOf(false) }
            TextAction("How it works", onClick = { showHowItWorks = true })
            TextAction("Open-source licenses & artwork", onClick = { showLicenses = true })
    ...
    if (showLicenses) {
        val notices = remember(context) {
            listOf("Spoiler Alert" to "Spoiler-Alert-MIT.txt", "DM Sans" to "DM-Sans-OFL.txt",
                "Roboto Condensed" to "Roboto-Condensed-Apache-2.0.txt").joinToString("\n\n") { (name, file) ->
                "$name\n\n" + context.assets.open("licenses/$file").bufferedReader().use { it.readText() }
            }
        }
        AlertDialog(
            onDismissRequest = { showLicenses = false },
            title = { Text("Open-source licenses & artwork") },
            text = {
                SelectionContainer {
                    Column(Modifier.heightIn(max = 420.dp).verticalScroll(rememberScrollState()),
                        verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        Text("Built for personal use and shared as a starting point. Make it your own and feel free to improve it.")
                        Text("Illustrated headers were generated with ChatGPT and Gemini. They are shared under the project terms to the extent the author holds rights. Fonts keep their own licenses below.")
