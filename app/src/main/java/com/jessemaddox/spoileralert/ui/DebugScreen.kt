...
fun DebugScreen(onBack: () -> Unit) {
    ...
    Column(
        ...
        Button(onClick = { post("Rowan", "This invented Falcons update is for the demo.") }) {
            Text("Fake group text (Falcons)")
        }
        Button(onClick = {
            postGroupChat("Demo sports chat", "Rowan", "This invented Falcons update is for the demo.")
        }) {
            Text("Fake demo chat burst (Falcons, tap several times)")
        }
        ...
        Button(onClick = { post("Weather", "Light rain is expected tomorrow") }) {
        ...
        Button(onClick = {
            scope.launch {
                checkInTestStatus = runCatching {
                    ...
                    val token = SessionToken.from(shield)?.let { ShieldRepository(context).adjustDeadlineForDebug(it, newArmedAt) }
                        ?: return@runCatching "This protection session has ended"
                    WorkManager.getInstance(context).enqueueUniqueWork(
                        "checkin-debug",
                        ExistingWorkPolicy.REPLACE,
