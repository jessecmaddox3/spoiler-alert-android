...
class MainActivity : ComponentActivity() {
    ...
    private var stopProtectionRequest by mutableStateOf<Long?>(null)
    ...
    private fun MainScaffold() {
        ...
        var statusGame by remember { mutableStateOf<ProtectedLiveGame?>(null) }
        ...
        val shields by vm.shields.collectAsStateWithLifecycle(emptyList())
                ...
                when {
                    ...
                    tab == 0 -> ProtectionHomeScreen(
                        ...
                        onEndProtection = { shield ->
                            ...
                            vm.endProtection(shield.id) { outcome ->
                        ...
                        onExtend = { shield ->
                            vm.extend(shield.id)
                            scope.launch {
                                snackbar.showSnackbar("Extended by 1 hour")
                            }
                        },
                        onOpenGameStatus = { statusGame = it },
                        onProtectAnotherGame = {
                            gamesTodayOnly = dailyFeed.otherToday.count > 0
                            showGames = true
                        },
        ...
        val stopShield = shields.firstOrNull { it.id == stopProtectionRequest }
        if (stopShield != null) {
            AlertDialog(
                ...
                confirmButton = {
                    TextButton(onClick = {
                        val releaseStarted = System.currentTimeMillis()
                        vm.endProtection(stopShield.id) { outcome ->
                            if (outcome != null && outcome.revealedCount > 0) {
                                catchUpSinceMillis = releaseStarted
                                showHistory = true
                            }
    ...
    private fun captureNotificationIntent(intent: Intent) {
        ...
        stopProtectionRequest = intent.getLongExtra(EXTRA_SHIELD_ID, -1L)
            .takeIf {
                it >= 0 && trustedAction && intent.action == ACTION_CONFIRM_STOP_PROTECTION
            }
