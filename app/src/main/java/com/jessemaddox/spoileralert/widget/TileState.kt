...
data class TileState(
    ...
    companion object {
        ...
        suspend fun snapshot(context: Context, repo: ShieldRepository = ShieldRepository(context)): TileState {
            ...
            val records = repo.shieldDao.protectionRecords()
            val shields = records.map { it.shield }
            val armedShields = records.filter { it.isEffective(System.currentTimeMillis()) }.map { it.shield }
            if (armedShields.isNotEmpty()) {
                ...
                return TileState(
                    ...
                    activeExpiresAtMillis = soonest?.let { if ((ShieldCodec.sessionDeadlineMillis(it) ?: 0) > System.currentTimeMillis())
