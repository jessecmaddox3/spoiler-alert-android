...
class DaoTest {
    ...
    @Test fun vaultRevealFlow() = runBlocking {
        ...
        dao.insert(VaultEntity(shieldId = 1, sourcePackage = "com.whatsapp",
            sourceAppLabel = "WhatsApp", title = "Kait", text = "did you see it",
    ...
    @Test fun sameGameCachedForTwoShieldsDoesNotCollide() = runBlocking {
        ...
        val sharedEventId = "401547439" // e.g. ATL @ NO
        dao.upsertAll(listOf(
            GameEntity(id = sharedEventId, shieldId = falconsShieldId, name = "Saints at Falcons",
                shortName = "NO @ ATL", startMillis = 1_000L, completed = false, fetchedAtMillis = 0L),
            GameEntity(id = sharedEventId, shieldId = saintsShieldId, name = "Saints at Falcons",
                shortName = "NO @ ATL", startMillis = 1_000L, completed = false, fetchedAtMillis = 0L),
        ))
    ...
    @Test fun gameShieldLookupByEventIdAndVaultCount() = runBlocking {
        val id = db.shieldDao().insert(ShieldEntity(
            name = "ENG @ FRA", aliasesJson = "[]", kind = "GAME", gameEventId = "760516",
        ))
        assertEquals(id, db.shieldDao().byGameEventId("760516")?.id)
    ...
    @Test fun multiDayEventRemainsUpcomingUntilItsEnd() = runBlocking {
        ...
        dao.upsertAll(listOf(
            LeagueGameEntity(
                eventId = "open", leagueId = "golf", name = "The Open", shortName = "The Open",
